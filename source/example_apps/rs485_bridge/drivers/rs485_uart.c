 ...
 *
 * UARTE20 is in the peripheral domain and accepts P1 / P2 pins.
 * It is NOT aliased by the Wirepas debug-UART HAL (which uses UARTE30 via the
 * board/sensorv26/usart_dma_series.h override), so both UARTs coexist.
 *
 ...
 *   → BOARD_RS485_UART_TX_PIN = 36 = (1<<5)|4 = P1.04
 *
...
void rs485_uart_init(void)
{
    ...
    NRF_UARTE20->PSEL.TXD = BOARD_RS485_UART_TX_PIN;
    NRF_UARTE20->PSEL.RXD = BOARD_RS485_UART_RX_PIN;
    ...
    NRF_UARTE20->FRAMETIMEOUT = 1000U;
...
bool rs485_uart_send(const uint8_t * data, uint8_t len)
{
    ...
    NRF_UARTE20->TASKS_DMA.RX.STOP = 1;

    /* Clear events, arm TX DMA, start. */
    NRF_UARTE20->EVENTS_DMA.TX.END  = 0;
    NRF_UARTE20->EVENTS_TXSTOPPED   = 0;
    NRF_UARTE20->DMA.TX.PTR         = (uint32_t)(uintptr_t)data;
    NRF_UARTE20->DMA.TX.MAXCNT      = len;
    NRF_UARTE20->TASKS_DMA.TX.START = 1;

    /* Wait for DMA to finish reading RAM → TX FIFO. */
    volatile uint32_t to = RS485_TX_DMA_TIMEOUT_ITER;
    while (!NRF_UARTE20->EVENTS_DMA.TX.END && to > 0)
    {
        to--;
    }
    bool dma_ok = (to > 0u);
    ...
    NRF_UARTE20->TASKS_DMA.TX.STOP = 1;
    to = RS485_TX_STOP_TIMEOUT_ITER;
    while (!NRF_UARTE20->EVENTS_TXSTOPPED && to > 0)
    {
        to--;
    }
...
void rs485_uart_rx_arm(void)
{
    ...
    NRF_UARTE20->TASKS_DMA.RX.STOP  = 1;
    NRF_UARTE20->EVENTS_DMA.RX.END  = 0;
    NRF_UARTE20->EVENTS_FRAMETIMEOUT = 0;
    for (uint32_t i = 0; i < RS485_RX_BUF_LEN; i++)
    {
        m_rx_buf[i] = 0u;
    }
    NRF_UARTE20->DMA.RX.PTR    = (uint32_t)(uintptr_t)m_rx_buf;
    NRF_UARTE20->DMA.RX.MAXCNT = RS485_RX_BUF_LEN;
    NRF_UARTE20->TASKS_DMA.RX.START = 1;
}

bool rs485_uart_rx_event(void)
{
    return (NRF_UARTE20->EVENTS_FRAMETIMEOUT != 0u)
        || (NRF_UARTE20->EVENTS_DMA.RX.END   != 0u);
}
...
uint8_t rs485_uart_rx_stop(void)
{
    ...
    while (!NRF_UARTE20->EVENTS_DMA.RX.END && to > 0u) { to--; }
    uint8_t amt = (uint8_t)(NRF_UARTE20->DMA.RX.AMOUNT & 0xFFu);
    NRF_UARTE20->EVENTS_FRAMETIMEOUT = 0;
    NRF_UARTE20->EVENTS_DMA.RX.END   = 0;
    return amt;
}
...
void rs485_uart_rx_resume(uint8_t offset)
{
    ...
    NRF_UARTE20->DMA.RX.PTR    = (uint32_t)(uintptr_t)(m_rx_buf + offset);
    NRF_UARTE20->DMA.RX.MAXCNT = (uint16_t)(RS485_RX_BUF_LEN - offset);
    NRF_UARTE20->EVENTS_FRAMETIMEOUT = 0;
    NRF_UARTE20->EVENTS_DMA.RX.END   = 0;
    NRF_UARTE20->TASKS_DMA.RX.START  = 1;
}
