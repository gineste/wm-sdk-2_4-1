...
#include "i2c.h"      /* low-level I2C HAL */
#include "cbor.h"

/* ── canopee_v27 GPIO compatibility ───────────────────────────────────────────
 * This app descends from the sensorv26 rs485_bridge, whose board had two LEDs,
 * a push-button and a VBAT_EXT gate. canopee_v27 has a single LED, a fuel-gauge
 * ALERT input (reused as the "button" edge source) and an EXT supply enable +
 * fault. Map the old GPIO IDs onto the new ones so the shared code compiles. */
#define BOARD_GPIO_ID_LED_GREEN        BOARD_GPIO_ID_LED
#define BOARD_GPIO_ID_LED_RED          BOARD_GPIO_ID_LED
#define BOARD_GPIO_ID_SWITCH           BOARD_GPIO_ID_GAUGE_ALERT
#define BOARD_GPIO_ID_VBAT_EXT_EN      BOARD_GPIO_ID_EXT_EN
#define BOARD_GPIO_ID_VBAT_EXT_nFAULT  BOARD_GPIO_ID_EXT_FAULT
...
#define USE_ADS1220  0   /* no ADS1220/CTN/SP110 on canopee_v27 */
...
static void send_gauge_cbor_uplink(void)
{
    ...
    app_lib_data_to_send_t pkt = {
        ...
        .src_endpoint  = EP_GAUGE_CBOR,
        .dest_endpoint = EP_GAUGE_CBOR,
...
static uint32_t gauge_monitor_task(void)
{
    ...
    max17261_res_e r = MAX17261_read(&m_gauge);
    if (r != MAX17261_RES_OK)
    {
        LOG(LVL_WARNING, CRED "MAX17261 read error %d — will re-init" C0, (int)r);
        m_gauge_ready = false;
        return GAUGE_MONITOR_PERIOD_MS;
    }
...
#if defined(USE_MAX17261)
    /* Periodic MAX17261 fuel-gauge read + log + EP08 uplink. */
    App_Scheduler_addTask_execTime(gauge_monitor_task,
                                   APP_SCHEDULER_SCHEDULE_ASAP,
                                   GAUGE_MONITOR_EXEC_US);
#endif
